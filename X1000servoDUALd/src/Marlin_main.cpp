...
// M19  - Resume SD print from current (or given) Z height (disables all movements below the current Z position, a file must be selected to print after executing this M code)
// M20  - List SD card
// M21  - Init SD card
// M22  - Release SD card
...
// M220 S<factor in percent> or P<incremental percentage>- set speed factor override percentage
// M221 S<factor in percent> or P<incremental percentage>- set extrude factor override percentage
...
#define DEBOUNCE_INTERVAL 1000 //wait 1s for button pin to settle
unsigned long timeOfLastButtonEvent = 0; //store the last time the button state changed
boolean target_temp_reached = false;

extern float planner_disabled_below_z;
boolean change_filament = false;

boolean resume_backup_extruder = false;
// volatile bool feedsense_ready = false;
// volatile unsigned long last_feedsense = millis();
...
{
  if(card.sdprinting)
    feedsense_ready = true;
  last_feedsense = millis();
}
*/
...
void setup()
{
  ...
  SET_INPUT(FEED_PIN);
  cli();
  PCICR |= 0b00000001; // enable port B pin change interrupt
  PCMSK0 |= 0b00100000; // mask for bit 5 of port B (PB5/OC1A/PCINT5/physical pin 11)
  sei();
  */
  ...
  if(code_seen('G'))
  {
    ...
    case 1: // G1
      if(Stopped == false) {
        ...
        /*
        if(code_seen('E') && feedsense_ready && last_feedsense + 5000 < millis())
        {
          feedsense_ready = false;
          enquecommand_P(PSTR("M600"));
        }
        */
        if(code_seen('E'))
        {
          currentState = READ(PAUSE_PIN);
          if(currentState != lastState)
            timeOfLastButtonEvent = millis();
          if(currentState != debouncedState && millis() - timeOfLastButtonEvent > DEBOUNCE_INTERVAL) //if enough time has passed and the current state is still different than our last stored debounced state
          {
             debouncedState = currentState; //update the debounced state trigger an event
             if(!currentState)
               enquecommand_P(PSTR("M600"));
          }
    ...
    case 5: // Babystep X Y Z
    {
      ...
      for(unsigned short i=0; i<3; i++){ // in the order: Z, X, then Y
        ...
        if(axis[i] == Z_AXIS) // will move to the given (EEPROM saved) babystepped Z height offset when homing but not recognize it
          add_homing[axis[i]] = Z_BABY_DEFAULT_MIN_POS - baby_min_endstop[axis[i]];
      }
    }
    break;
    #endif //BABYSTEPPING
    ...
    case 28: //G28 Home all Axis one at a time
      ...
      if (planner_disabled_below_z) break; // Disable homing if resuming print
...
Sigma_Exit:
    ...
    {// M109 - Wait for extruder heater to reach target.
      ...
      target_temp_reached = false;
      //LCD_MESSAGEPGM(MSG_HEATING);   //BW COMMENTED OUT SO THAT 'HEATING....' IS NOT DISPLAYED DURING PRINTING
      #ifdef AUTOTEMP
      ...
      cancel_heatup = false;
        ...
        //LCD_MESSAGEPGM(MSG_HEATING_COMPLETE);    //BW COMMENTED OUT SO THAT 'HEATING COMPLETE....' IS NOT DISPLAYED DURING PRINTING
        starttime=millis();
        previous_millis_cmd = millis();
      ...
      case 106: //M106 Fan On
        if (code_seen('S')){
           if(code_value() > 0)
             ...
             fanSpeed=0;
           fanSpeed=constrain(code_value(),0,255); // RANGE OF PWM DUTY CYCLE FAN SPEEDS (disabled due to static friction/damage at the fan's plastic, not necessary with fan shroud ?)
        }
        else {
          fanSpeed=255;
        }
    ...
    break;
    case 206: // M206 additional (babystepped) homing offset
      for(int8_t i=0; i < 3; i++)
      {
    ...
    #endif
    case 220: // M220 S<factor in percent> or P<incremental percentage>- set speed factor override percentage
    {
      if(code_seen('S'))
      {
    ...
    break;
    case 221: // M221 S<factor in percent> or P<incremental percentage>- set extrude factor override percentage
    {
      if(code_seen('S'))
      {
...
case 404:  //M404 Enter the nominal filament width (3mm, 1.75mm ) N<3.0> or display nominal filament width 
    ...
    case 600: //Pause for filament change X[pos] Y[pos] Z[relative lift] E[initial retract] L[later retract distance for removal]
    {
        ...
        delay(100);
        if(resume_backup_extruder)
          LCD_MESSAGEPGM(MSG_EXTRUDERSWITCHED);
        else
        LCD_MESSAGEPGM(MSG_FILAMENTCHANGE);
        uint8_t cnt=0;
        unsigned long alarm_time = millis();
        float extruder0_temp = degTargetHotend0();
        #if EXTRUDERS > 1
        float extruder1_temp = degTargetHotend1();
        #endif
        while(!lcd_clicked() && change_filament && !resume_backup_extruder)
		{
          cnt++;
          manage_heater();
          manage_inactivity();
          ...
          if(millis() > alarm_time + 1800000)
          {
            if(millis() < alarm_time + 1801000)
            {
              setTargetHotend0(0.0);
              #if EXTRUDERS > 1
              setTargetHotend1(0.0);
              #endif
        ...
        //return to normal
        if((degTargetHotend0() == 0 && extruder0_temp > 23) || resume_backup_extruder)
        {
          if(resume_backup_extruder)
          {
          setTargetHotend0(extruder1_temp);

          enquecommand_P(PSTR("M109 T1"));
          }
          else
            setTargetHotend0(extruder0_temp);
  enquecommand_P(PSTR("M109 T0"));		
        }
        #if EXTRUDERS > 1
        if((degTargetHotend1() == 0 && extruder1_temp > 23) || resume_backup_extruder)
        {
          if(resume_backup_extruder)
          {
            setTargetHotend1(extruder0_temp);
            if(active_extruder)
              enquecommand_P(PSTR("M109 T1"));
          }
          else
            setTargetHotend1(extruder1_temp);
        ...
        if(resume_backup_extruder)
        {
          ...
          for(int i = 0; i < 2; i++)
            current_position[i] = current_position[i] - extruder_offset[i][active_extruder] + extruder_offset[i][!active_extruder];
    ...
    case 602:
      if(code_seen('S'))
        resume_backup_extruder = code_value();
    #endif
    #ifdef DUAL_X_CARRIAGE
