...
extern bool resume_backup_extruder;
...
static void lcd_status_screen()
{
    ...
    if(LCD_CLICKED) lcd_implementation_init();
...
void lcd_set_home_offsets()
{
  ...
  static void _lcd_babystep(int axis, const char *msg) {
    ...
    else if (lcdDrawUpdate && axis == Z_AXIS) lcd_implementation_drawedit(msg, ftostr50(add_homing[axis]));
...
static void lcd_tune_menu()
{
    ...
    MENU_ITEM(submenu, MSG_BABYSTEP_Z, lcd_babystep_z);
     ...
     MENU_ITEM_EDIT(bool, MSG_USEBACKUPEXTRUDER, &resume_backup_extruder);
#endif
    END_MENU();
}
...
#endif
    MENU_ITEM(function, VERSION, lcd_prepare_menu);
    MENU_ITEM(function, MSG_CLEAR_BABYSTEP, lcd_set_home_offsets);
...
static void lcd_control_motion_menu()
{
    ...
    #if EXTRUDERS > 1
    MENU_ITEM_EDIT(float52, MSG_XOFFSET, &extruder_offset[X_AXIS][1], 100, 150);
    MENU_ITEM_EDIT(float52, MSG_YOFFSET, &extruder_offset[Y_AXIS][1], -5, 5);
    #endif // EXTRUDERS > 1
...
menu_edit_type(float, float50, ftostr50, 100)
...
// Convert float to string with +12.34 format
char *ftostr50(const float &x)
{
  long xx=x*100;
  conv[0]=(xx>=0)?'+':'-';
  xx=abs(xx);
